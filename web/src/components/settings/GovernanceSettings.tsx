/** GovernanceSettings — external project governance health panel. */
...
export function GovernanceSettings() {
  ...
  const fetchHealth = useCallback(async () => {
    ...
    setError(null);
    try {
      const res = await fetch(`${API_BASE}/api/governance/health`);
      if (!res.ok) {
        setError("加载治理状态失败");
        return;
      }
  ...
  if (projects.length === 0) {
    return (
      <div className="py-8 text-center text-gray-400">
        ...
        <p className="mt-1 text-xs">当猫咪首次被派遣到外部项目时，治理规则会自动同步</p>
      </div>
    );
  }
